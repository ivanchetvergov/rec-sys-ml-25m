"""Explicit ALS Recommender (ExplicitALSRecommender) — Collaborative Filtering baseline.

Solves the explicit rating prediction problem via Truncated SVD (closed-form
solution to the explicit ALS objective when alternating updates converge to
a single SVD step on the centered rating matrix).

    R_centered ≈ U Σ Vᵀ   (truncated to k factors)
    score(u, i) = μ_u + (U[u] · Σ) · V[i]

Key difference from ImplicitALSRecommender (iALS):
    - Trains only on OBSERVED ratings (missing entries are ignored)
    - Optimises MSE on actual rating values, not binary preferences
    - Cannot naturally leverage implicit feedback (views, clicks, etc.)
    - Used as a reference baseline; iALS is used in the production pipeline

Usage:
    model = ExplicitALSRecommender(factors=100, random_state=42)
    model.fit(train_df)
...
class ExplicitALSRecommender:
    """Explicit ALS (Matrix Factorization) recommender via Truncated SVD.

    Decomposes the mean-centered user-item ratings matrix R_centered ≈ U Σ Vᵀ,
    then scores an unseen (user, item) pair as:
        score(u, i) = μ_u + (U[u] * Σ) · V[i]

    where μ_u is the user’s mean rating (used for centering).

    Only observed ratings contribute to the loss — missing entries are simply
    ignored.  This is the explicit-feedback counterpart of ImplicitALSRecommender.
    ...
    def fit(
        ...
        """Fit Explicit ALS on the user-item ratings matrix.
        ...
        Returns:
            self (for chaining)
        """
        logger.info("Building user-item matrix for Explicit ALS (SVD)...")
        ...
        logger.info(
            f"Explicit ALS (SVD) complete. user_factors shape: {self.user_factors_.shape}, "
    ...
    def save(self, path: Path) -> None:
        ...
        logger.info(f"ExplicitALSRecommender saved to {path}")

    @classmethod
    def load(cls, path: Path) -> "ExplicitALSRecommender":
        """Load a saved ExplicitALSRecommender from directory.

        Args:
            path: Directory path written by :meth:`save`.

        Returns:
            Loaded ExplicitALSRecommender instance.
        """
        path = Path(path)
        ...
        expected_type = "ExplicitALSRecommender"
        if config.get("model_type") != expected_type:
            raise ValueError(
                f"model_config.json reports model_type='{config.get('model_type')}', "
                f"expected '{expected_type}'. "
                "Are you loading an ExplicitALSRecommender from the right directory?"
            )
        ...
        logger.info(
            f"ExplicitALSRecommender loaded from {path} "
    ...
    def __repr__(self) -> str:
        ...
        return f"ExplicitALSRecommender({status}{details})"


# Backward-compatibility alias — old code importing SVDRecommender still works
SVDRecommender = ExplicitALSRecommender

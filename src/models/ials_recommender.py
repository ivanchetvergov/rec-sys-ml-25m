...
Unlike explicit ALS (which minimises MSE only on observed ratings), iALS
treats every user-item pair as a preference signal:
    - Preference p_ui = 1 if the user interacted with item i, else 0
    - Confidence c_ui = 1 + α · f(r_ui)  (observed) / 1 (unobserved)
    - Optimised over the FULL matrix with weighted RMSE

Supported confidence modes:
    'linear' : c_ui = 1 + alpha * r_ui          (standard iALS)
    'log'    : c_ui = 1 + alpha * log1p(r_ui)   (log-confidence, less popularity bias)
    'binary' : c_ui = alpha for all rated items  (ignores rating magnitude)

Usage:
    model = ImplicitALSRecommender(factors=128, iterations=20, alpha=15.0)
    model.fit(train_df)
...
class ImplicitALSRecommender:
    """Implicit ALS (iALS) candidate generator — Stage 1 of the two-stage pipeline.

    Optimises over the full user-item matrix using confidence-weighted binary
    preferences.  Unobserved pairs are treated as weak negatives (confidence=1)
    rather than ignored, which is the key difference from explicit ALS / SVD.
    ...
    Args:
        ...
        confidence_mode: How to compute confidence from ratings.
            ...
            'log'     → c = 1 + alpha * log1p(rating)  (recommended: less popularity bias)
    ...
    def __init__(
        ...
        alpha: float = 15.0,
        confidence_mode: ConfidenceMode = "log",
        random_state: int = 42,
        num_threads: int = 0,
    ) -> None:
    ...
    def fit(
        ...
        # Prevent OpenBLAS from spawning threads that compete with implicit's
        # own parallelism — without this, ALS can be ~2× slower on some systems.
    ...
    def save(self, path: Path) -> None:
        ...
        config = {
            "model_type": "ImplicitALSRecommender",
            "factors": self.factors,
            "iterations": self.iterations,
            "regularization": self.regularization,
            "alpha": self.alpha,
        ...
        logger.info(f"ImplicitALSRecommender saved to {path}")

    @classmethod
    def load(cls, path: Path) -> "ImplicitALSRecommender":
        """Load ImplicitALSRecommender from directory written by :meth:`save`."""
        path = Path(path)
        with open(path / "model_config.json") as f:
            config = json.load(f)

        expected_type = "ImplicitALSRecommender"
        if config.get("model_type") != expected_type:
            raise ValueError(
                f"model_config.json reports model_type='{config.get('model_type')}', "
                f"expected '{expected_type}'. "
                "Are you loading an ImplicitALSRecommender from the right directory?"
            )
        ...
        logger.info(
            f"ImplicitALSRecommender loaded from {path} "
    ...
    def __repr__(self) -> str:
        ...
        return f"ImplicitALSRecommender({status}{details})"


# Backward-compatibility alias — old code importing ALSRecommender still works
ALSRecommender = ImplicitALSRecommender

...
export default function OnboardingPage() {
    ...
    const saveAll = async () => {
        ...
        for (const m of movies) {
            ...
            if (!r) continue
            await upsertReview(token, m.id, r, '')
    ...
    return (
        <div className='min-h-screen pt-24 pb-16 px-4 md:px-8' style={{ background: 'var(--bg-primary)' }}>
            <div className='max-w-4xl mx-auto'>
                <h1 className='text-3xl font-black text-white'>{tr.onboarding.heading}</h1>
                <p className='text-zinc-400 mt-2'>{tr.onboarding.subheading}</p>
                ...
                <div className='mt-6 flex items-center gap-3'>
                    ...
                    >
                        {saving ? tr.onboarding.saving : tr.onboarding.continue(ratedCount)}
                    </button>
                    <button
                        onClick={() => router.push('/')}

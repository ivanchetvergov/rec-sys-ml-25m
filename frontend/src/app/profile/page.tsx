...
// ── Watched card — uses WatchedItem data directly (no extra fetch needed) ────
function WatchedCard({
	...
	const movie: Movie = {
		...
		num_ratings: item.num_ratings ?? null,
		...
		<MiniCard
			...
			badge={
				<span
					...
					style={{ background: 'rgba(46,160,67,0.85)', color: '#fff' }}
				>
					✓
				</span>
			}
...
export default function ProfilePage() {
	...
	async function handleRemoveWatched(movieId: number) {
		const token = getToken()
		if (!token) return
		await removeWatchedDB(token, movieId)
		setWatched(prev => prev.filter(e => e.movie_id !== movieId))
	}
	...
	return (
		...
		>
			...
			<div
				...
				style={{
					background:
						'linear-gradient(to bottom, rgba(229,9,20,0.04) 0%, transparent 100%)',
				}}
			/>
					...
					>
						{watched.map(item => (
							<WatchedCard
								key={item.movie_id}
								item={item}
								onRemove={() => handleRemoveWatched(item.movie_id)}
							/>
						))}
					</MovieRow>
				</Section>
					...
					{reviews.length === 0 ? (
						...
						<div className='flex flex-col gap-4'>
							...
							<div className='flex flex-col gap-3'>
								{reviews.map(rv => {
									const gradient =
										CARD_GRADIENTS[rv.movie_id % CARD_GRADIENTS.length]
									return (
										<div
											key={rv.movie_id}
								...
								{genreStats.slice(0, 5).map(({ genre, count }, i) => {
									...
									return (
										<div key={genre} className='flex items-center gap-2'>
											...
											<span className='text-xs text-zinc-600 ml-auto pl-4'>
												{count}×
											</span>
										</div>
									)
